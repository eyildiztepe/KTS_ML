...
güçsüzlüğü ve körelme meydana gelir . Bu hastalarda elde, özellikle aktivite ile artan beceriksizlik ve objeleri kavramada kuvvetsizlik görülür (Aroori ve Spence, t.y.).

\hypertarget{KTSTani}{%
\section{Tanı}\label{KTSTani}}

Karpal tünel sendromunda tanı koymak için hastanın hikayesi, klinik semptomlar, fizik muayene bulguları ve
bu bulguların elektronörofizyolojik testlerle desteklenmesi kullanılmaktadır (Ghasemi-Rad ve diğerleri, 2014).
...
Anketi kullanılmaktadır örnek olarak Türkiyeden Melek Sezgin ve arkadaşlarının boston anketi ile karpal tünel sendromu için yapmış oldukları ciddiyet sınıflandırması gösterilebilir(Sezgin ve diğerleri, 2006).
...
2020 yılında Ali Abbasian Ardakani ve arkadaşları tarafından hasta olduğu bilinen kişilerden elde edilen bilgisayarlı tomografi görüntüleri, derin öğrenme metodları kullanılarak başka kişilerin hasta olup olmadığını tespit etmek için kullanılmıştır(Ardakani ve diğerleri, 2020).\\
Bir diğer çalışma ise 2021 yılında Takafumi Koyama ve arkadaşları tarafından geliştirilen bir mobil uygulama sayesinde hastaların ekranın farklı yerlerinde çıkan cisimlere ulaşma sürelerini baz alarak hastalığın evresini tahminlemeyi amaçlamıştır(Koyama ve diğerleri, 2021). Bu uygulumanın deneysel olması ile birlikte hastanın kendi kendine ev ortamında hastalığına ön tanı koyabilmesi açısından yararlı olduğu düşünülmektedir.\\
Son olarak bizimde ilgilendiğimiz bir örnek olarak 2021 yılında Dougho Park ve arkadaşları tarafından 1035 hastaya yapılan testlerin sonuçlarını farklı makine öğrenmesi algoritmalarında kullanarak hastaların, hastalığının ciddiyetini sınıflandırmak ve doktor gözetiminde gerekli tedavinin belirlenebilmesine yardımcı olması amaçlanmıştır(Park ve diğerleri, 2021).

\hypertarget{r-markdown-tanux131tux131mux131}{%
\section{R Markdown Tanıtımı}\label{r-markdown-tanux131tux131mux131}}
...
Aroori, S. ve Spence, R. (t.y.). Carpal Tunnel Syndrome. 2008. \emph{The Ulster Medical Society}, \emph{77}, 1-17.
