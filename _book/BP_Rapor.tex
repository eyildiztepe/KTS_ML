...
Rassal ormanlar sınıflandırıcısı, her biri farklı girdi vektörlerinden oluşan ve her ağacın yalnızca bir sınıfa oy verdiği karar ağaçlarının kombinasyonudur (Breiman, 1999). Rassal ormanlar sınıflandırması ağaç derinliğini büyütmek için her dalda rastgele değişkenleri içerir.

Karar ağaçlarını modellemek, bir budama metodu ve nitelik seçme ölçütü seçmeyi gerektirir. Karar ağacının tüme varımı için kullanılan bir sürü nitelik seçme yaklaşımı vardır ve çoğu yaklaşımlar niteliğe direkt olarak kalite ölçümü belirler. Karar ağacının tümevarımındaki en sık kullanılan nitelik seçme ölçümleri Information gain ratio kriteri ve Gini indexidir (Breiman, Friedman, Olshen ve Stone, 2017).
...
\hypertarget{gradyan-artux131rux131mux131}{%
\subsection{Gradyan Artırımı}\label{gradyan-artux131rux131mux131}}

Gradyan artırma fikri, Leo Breiman'ın , artırmanın uygun bir maliyet fonksiyonu üzerinde bir optimizasyon algoritması olarak yorumlanabileceği gözleminden kaynaklanmıştır (Breiman, 1997). Açık regresyon gradyan artırma algoritmaları daha sonra Jerome H. Friedman (Jerome H. Friedman, 2001) tarafından, Llew Mason, Jonathan Baxter, Peter Bartlett ve Marcus Frean'ın daha genel fonksiyonel gradyan artırma perspektifiyle eş zamanlı olarak geliştirilmiştir (Mason, Baxter, Bartlett ve Frean, 1999).
...
\caption{Gradyan Artırma Çalışma Mantığı (Dikker, 2017)}\label{fig:unnamed-chunk-8}
\end{figure}
\hypertarget{xgboost-aux15fux131rux131-gradyan-artux131rux131mux131}{%
\subsection{XGBoost (Aşırı Gradyan Artırımı)}\label{xgboost-aux15fux131rux131-gradyan-artux131rux131mux131}}

XGBoost, temeli gradyan artırımı ve karar ağacı algoritmalarına dayanan denetimli makine öğrenmesi tekniğidir.
XGBoost algoritmasının orijinal hali Friedman tarafından 2002 yılında geliştirilmiştir (Jerome H. Friedman, 2002). Sonrasında Washington Üniversitesi'nde iki araştırmacı olan Tianqi Chen ve Carlos Guestrin tarafından SIGKDD (Special Interest Group on Knowledge Discovery and Data Mining) 2016 konferansında bildiri olarak sunulmuştur ve makine öğrenme dünyasında çok popüler olmuştur (Chen ve Guestrin, 2016).

XGBoost, yüksek tahmin etme gücüne sahip olup,bazı çalışmalarda diğer algoritmalardan daha hızlı olduğu bilinmektedir. Ayrıca XGBoost, genel performansı iyileştiren ve aşırı uyum ya da aşırı öğrenmeyi azaltan bir dizi düzeltme işlemleri içermektedir (Yangın, 2019).
...
Uygulamada Python programlama dili ve Scikit-Learn (Buitinck ve diğerleri, 2013), Pandas, Numpy, Matplotlib, XGBoost kütüphanelerinden yararlanılmıştır. Kullanılan kodlar ve veri seti açık kaynaklı olarak \href{https://github.com/eyildiztepe/KTS_ML}{KTS\_ML} adı altında github deposunda paylaşılmaktadır.
...
\end{verbatim}
\begin{verbatim}
Balanced Accuracy Score : 0.8054630657294364
\end{verbatim}
\begin{figure}
...
\hypertarget{refs}{}
\begin{CSLReferences}
\hspace{0.62cm}\leavevmode\vadjust pre{\hypertarget{ref-ardakani2020diagnosis}{}}%
...
Breiman, L. (1997). \emph{Arcing the edge}. Technical Report 486, Statistics Department, University of California.
...
Breiman, L., Friedman, J. H., Olshen, R. A. ve Stone, C. J. (2017). \emph{Classification and regression trees}. Routledge.
...
Friedman, Jerome H. (2001). Greedy function approximation: A gradient boosting machine. \emph{The Annals of Statistics}, \emph{29}(5).

\leavevmode\vadjust pre{\hypertarget{ref-friedman2002stochastic}{}}%
Friedman, Jerome H. (2002). Stochastic gradient boosting. \emph{Computational statistics \& data analysis}, \emph{38}(4), 367-378.
